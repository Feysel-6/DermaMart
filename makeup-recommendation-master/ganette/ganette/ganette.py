...
# pykeops and geomloss are optional - only needed for score() method
try:
    import pykeops
except ImportError:
    pykeops = None
...
class Ganette(ConditionalGenerativeModel, BaseEstimator, Picklable):
    class Generator(LoadableModule):
        def __init__(self, x_features, y_features, latent_size, layers):
            ...
            layer_sizes = np.linspace(latent_size + y_features, x_features, layers + 1).astype(int)
    ...
    class Discriminator(nn.Module):
        def __init__(self, x_features, y_features, layers, dropout_prob):
            ...
            layer_sizes = np.linspace(x_features + y_features, 1, layers + 1).astype(int)
    ...
    def score(self, x, y):
        check_is_fitted(self)
        if pykeops is None or SamplesLoss is None:
            raise ImportError("pykeops and geomloss are required for score() method. Install with: pip install pykeops geomloss")
